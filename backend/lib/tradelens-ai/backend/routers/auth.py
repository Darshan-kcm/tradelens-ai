...
async def _start_session(user: dict, response: Response) -> None:
    ...
    response.set_cookie(
        ...
        token,
        httponly=True,
        # "lax" works because the Vercel rewrite in frontend/vercel.json proxies
        # /api/* to this backend, so the browser only ever sees ONE origin
        # (the Vercel domain) — this is no longer a cross-site request. Keeping
        # this at "lax" (rather than "none") avoids depending on third-party
        # cookie support, which many browsers now restrict or block by default.
        samesite="lax",
        secure=True,
