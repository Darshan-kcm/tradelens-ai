// Typed fetch layer over the FastAPI backend. In dev, Vite proxies /api -> :8001 so a
// relative path works. In production the frontend and backend are usually on different
// domains (e.g. Vercel + Render), so VITE_API_URL points at the deployed backend origin;
// leave it unset to keep using the relative "/api" path (single-origin deployments).
...
async function request<T>(method: string, path: string, body?: JsonBody): Promise<T> {
  ...
  // credentials: "include" is required so the cookie is sent/stored when the frontend and
  // backend are on different origins (cross-site deployments like Vercel + Render).
